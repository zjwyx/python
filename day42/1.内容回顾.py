...
# 索引的创建和删除
    # create index ind_name_on 表名(字段名)
    # create index ind_name_on 表名(字段名，字段2)
    # drop index 索引 on 表名
...
# 正确的使用mysql数据库
    ...
    # 从操作数据的角度
        # 尽量在where字段就约束数值到一个比较小的范围：分页
            # where a between value and value2
    # 尽量使用连表查询代替子查询
...
# 覆盖索引：查询过程中不需要回表
# select id from 表 where id > 10000;
# select max(id) from 表 where id > 10000;
# select count(id) from 表 where id > 10000;

# 索引合并:分别创建的两个索引在某一次查询种临时合并成一条索引 a=1orb=2
# 执行计划：explain select语句 能够查看sql语句有没有按照预期执行，可以查看索引的使用情况，type等级
# 慢查询优化
    # 首先从sql的角度优化
